...
There are two intended ways to use the vignette. In the version built with the package, `full_reproduction` is `FALSE`. This uses `nboot = 10` and restricts the more expensive five-list finite searches to modest values of `ntop`: 20 for the ordinary ranking and 10 for the degree-2 ranking. These settings are enough to exercise the current code, produce the figures and tables in reduced-precision form, and keep an ordinary package or CRAN vignette build reasonably quick. They are not intended to reproduce the published Monte Carlo values exactly.
